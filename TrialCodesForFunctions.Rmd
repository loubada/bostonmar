...
top10percentmean <- function(dataset){
  top10 <- dataset %>% arrange(Official.Time.1) %>% head(round(length(dataset$Official.Time.1)*0.1)) %>% mutate(X0K.1 = rep(0)) %>% select("X0K.1", c(13:22)) %>% lapply(mean) %>% as.data.frame() %>% `row.names<-`(c("time")) %>% t() %>% as.data.frame() %>%  setDT(keep.rownames=TRUE) %>% mutate(Label = rep("Top10"))
  top10$rn <- c(0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50)
...
test
```
## 5. Top 10%, Bottom 20%, Global Average - Bottom 10%
```{r}
...
bottom20percentmean <- function(dataset){
  bottom20 <- dataset %>% arrange(desc(Official.Time.1)) %>% head(round(length(dataset$Official.Time.1)*0.2)) %>% mutate(X0K.1 = rep(0)) %>% select("X0K.1", c(13:22)) %>% lapply(mean) %>% as.data.frame() %>% `row.names<-`(c("time")) %>% t() %>% as.data.frame() %>%  setDT(keep.rownames=TRUE) %>% mutate(Label = rep("Bottom20"))
  bottom20$rn <- c(0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50)
  colnames(bottom20)[1]<-("milestone(km)")
  return(bottom20)
}  
...
globalaverage <- function(dataset){
  global <- dataset %>% mutate(X0K.1 = rep(0)) %>% select("X0K.1", c(13:22)) %>% lapply(mean) %>% as.data.frame() %>% `row.names<-`(c("time")) %>% t() %>% as.data.frame() %>%  setDT(keep.rownames=TRUE) %>% mutate(Label = rep("Global"))
  global$rn <- c(0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50)
  colnames(global)[1]<-("milestone(km)")
  return(global)
}
