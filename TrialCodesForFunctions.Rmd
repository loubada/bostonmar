...
agebracketcol <- function(X){
  X["Age_Bracket"] <- X["Age"] %>% agebracket() %>% as.vector()
  return(X)
}
...
test_top10 <- top10percentmean(data_all_dup)
test
```
...
test_bottom20 <- bottom20percentmean(data_all_dup)
test
```
...
test_average <- globalaverage(data_all_dup)
test
```
## 6. past time
```{r}
...
test_past <- pasttime(200, data_all_dup)
test
...
test_goal <- goaltime(200, data_all_dup)
test_goal


test <- rbind(test_average, test_bottom20, test_goal, test_past, test_top10)
test %>% ggplot(aes(x=milestone_km, y=mean_time, col=Label))+geom_point()+geom_line()
test_goal %>% ggplot(aes(x=milestone_km, y=mean_time, col=Label))+geom_point()+geom_line()
